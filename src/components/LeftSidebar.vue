<template>
  ...
  >
    ...
    <div v-for="item in data.items" :key="item.name" class="group relative">
      <!-- Parent -->
      <div
        class="card flex items-center justify-between gap-3 bg-white/5 border border-white/10 rounded-2xl p-4 text-white/80 font-semibold hover:text-amber-300 hover:border-amber-500/60"
      >
        <span>{{ item.name }}</span>
        <img
          v-if="item.logo"
          :src="`https://cdn.simpleicons.org/${item.logo}/f59e0b`"
          class="w-7 h-7"
          @error="hideImg"
        />
      </div>
    ...
    <teleport to="body">
      <div v-if="show" class="fixed inset-0 z-[9999] flex">
        ...
        >
          ...
          <p class="text-slate-300 mb-4">{{ active.desc }}</p>
          ...
          <div v-if="active.configs?.length" class="space-y-2 mb-4">
            ...
            >
              <span class="text-emerald-300 font-semibold cursor-pointer">
                {{ c.file }}
              </span>
          ...
          <!-- YAML -->
          <h3 class="text-amber-300 mb-2">YAML</h3>
...
function openDetails(item) {
  ...
  yamlText.value = "";
  show.value = true;
  if (item.configs?.length) loadYaml(item.configs[0].path);
...
onMounted(async () => {
  ...
  gsap.from(title.value, { y: -40, opacity: 0, delay: 0.3, duration: 0.7 });
});
</script>
